...
showInfoContent.innerHTML = `
  ...
  <div class="show-text">
    ...
    <p><strong>Status:</strong> <span id="showStatus">Loading...</span></p>
    <p><strong>Overview:</strong> ${overview || 'No overview available.'}</p>
  </div>
`;

// Fetch cast
fetch(`https://api.themoviedb.org/3/tv/${showId}/credits?api_key=4573eaa5baa0da8e3930d9cedca27140`)
  .then(res => res.json())
  .then(data => {
...
// Fetch season & episode count and show status
fetch(`https://api.themoviedb.org/3/tv/${showId}?api_key=1070730380f5fee0d87cf0382670b255`)
  ...
  .then(data => {
    ...
    showStatus.textContent = data.status || 'Unknown';
...
const ambientLighting = new AmbientLighting(videoWrapper, iframe);

// Start ambient lighting when the iframe loads
iframe.addEventListener('load', () => {
  ambientLighting.start();
});

// Stop ambient lighting when leaving the page
window.addEventListener('beforeunload', () => {
  ambientLighting.stop();
}); 